...
function toMemoryListRow(memory: Memory): MemoryListRow {
    return {
        id: String(memory.id),
...
function renderMemoryTableRows(rows: MemoryListRow[]): string {
    const headers = {
        id: "ID",
        category: "CATEGORY",
        status: "STATUS",
...
/** Render explicitly requested memories without list pagination semantics. */
export function formatMemoryTable(memories: Memory[]): string {
    if (memories.length === 0) {
        return "No active memories found.";
    }

    const rows = memories.map(toMemoryListRow);
    return [
...
export function formatMemoryList(page: MemoryListPage): string {
    ...
    const allRows = pageMemories.map(toMemoryListRow);
